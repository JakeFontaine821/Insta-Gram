...
app.get('/sender/album', (req, res) => {
    if(!req.query.albumName){ return res.json({ success: false, error: 'Missing required query parameter: \'albumName\'' }); }
    return res.json(AlbumDatabaseManager.getAlbum(req.query.albumName));
});
...
app.post('/sender/albums/rename', (req, res) => {
    if(!req.body.oldName){ return res.json({ success: false, error: 'Missing required field: \'oldName\'' }); }
    if(!req.body.newName){ return res.json({ success: false, error: 'Missing required field: \'newName\'' }); }
    return res.json(AlbumDatabaseManager.createNewAlbum(req.body.oldName, req.body.newName));
});
